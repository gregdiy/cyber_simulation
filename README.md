...
|---|---|---|
| `daniel.davis004` on `WS-SAL-0005` | Initial compromised user (Sales) | 127 |
| `sarah.wilson062` on `WS-IT-0063` | Pivot target (IT, normal account) — attacker + legitimate IT activity mixed | — |
| `sarah.wilson062_admin` on `WS-IT-0063` | Privilege context (IT, admin account) — admin-like attacker actions blended with normal admin work | — |
...
You can explore the dataset directly in the BigQuery web UI without any local setup.

**Steps:**

1. Open [BigQuery Studio](https://console.cloud.google.com/bigquery)
2. Run queries against: `diymind-dev.phantom_armor_benchmark.simulation`
...
**Privilege transitions are the hard part.** The pivot from Sales → IT → IT Admin puts attacker behavior inside the legitimate admin "shape."
...
| Attacker actions (`attack_id` non-null) | 345 (~0.004% of total) |
| Attack-triggered observability events | 285 (not de-duplicated) |
| Duration | 25 days |
| Users | 500 |
| Service accounts | 55+ |
...
- **For Commercial Use:** [Contact for licensing](mailto:gregralr@phantomarmor.com)
